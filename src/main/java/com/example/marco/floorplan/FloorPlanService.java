...
public class FloorPlanService {
    ...
    public FloorPlanEntity replaceFloorPlanEntityByFloorPlanId(FloorPlanEntity inFloorPlanEntity) throws Exception{
        if(inFloorPlanEntity.getFloorPlanId() == null){
            throw new Exception("replaceFloorPlanEntityByFloorPlanId errro: floorPlanId is null");
        }
        if(inFloorPlanEntity.getFloorId() == null){
    ...
    public FloorPlanEntity replaceFloorPlanEntityByFloorId(FloorPlanEntity inFloorPlanEntity) throws Exception{
        if(inFloorPlanEntity.getFloorPlanId() != null){
            throw new Exception("replaceFloorPlanEntityByFloorId error: cannot have explicit floorPlanId: " + inFloorPlanEntity.getFloorPlanId());
        }
        if(inFloorPlanEntity.getFloorId() == null){
        ...
        FloorPlanEntity toSaveFloorPlanEntity = null;
        if(optFloorPlanEntity.isEmpty()){
            toSaveFloorPlanEntity = new FloorPlanEntity();
        } else {
            toSaveFloorPlanEntity = optFloorPlanEntity.get();
        }
        toSaveFloorPlanEntity.setFloorId(inFloorPlanEntity.getFloorId());
        toSaveFloorPlanEntity.setFileId(inFloorPlanEntity.getFileId());
        return this.floorPlanRepository.save(toSaveFloorPlanEntity);
    }
